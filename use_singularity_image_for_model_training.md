...
（2）支持vnc的镜像,可以在镜像里面启动vncserver，默认启动的vnc端口是5909，vnc密码是vncpasswd,可以参考镜像中vncstartup下的readme文件修改默认端口和密码

# singularity build --sandbox centos_vnc centos_vnc.simg
# singularity shell --w centos_vnc
# cd /vncstartup
# ./vnc_startup.sh
（3）支持cuda的镜像，启动的时候必须带--nv
...
（1） 使用下面的命令，进入镜像，并在镜像内启动vncserver
...
# cd /vncstartup
# ./vnc_startup.sh
（2）在客户端使用vncviewer访问这个镜像，ip为host主机的ip，端口为5909，密码为vncpasswd

（3）vnc登录后， 在vnc桌面上点击pycharm，启动pycharm开发工具, pycharm的工程在/opt/project下面

（4）caffe安装在镜像的/opt目录下，详细使用见镜像/opt下的readme
...
（4）caffe2安装在镜像的/opt目录下，详细使用见镜像/opt下的readme
...
（3）vnc登录后， 在vnc桌面上点击pycharm，启动pycharm开发工具, pycharm的工程在/opt/project下面，工程就是物体识别的代码
